...
export async function* getCategories() {
	const categories = await fetch( { path: '/wp/v2/categories?per_page=-1' } );
...
export async function* getAuthors() {
	const users = await fetch( { path: '/wp/v2/users/?who=authors&per_page=-1' } );
...
export async function* getEntityRecord( state, kind, name, key ) {
	...
	const record = await fetch( { path: `${ entity.baseUrl }/${ key }?context=edit` } );
...
export async function* getEntityRecords( state, kind, name ) {
	...
	const records = await fetch( { path: `${ entity.baseUrl }?context=edit` } );
